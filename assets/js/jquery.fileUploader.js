	...
	$.fn.fileUploader = function(config){
		...
		var px = {
			...
			printForm: function(){
				...
				//This is not good but i left no choice because.on function is not working on IE
		...
		*/
		...
		//Button Clear Event
		$(pxButton).on('click',config.buttonClear, function(){
		...
		$(pxUploadForm).on('click','.delete', function(){
		...
		*/
		$(pxUploadForm).on('click','.cancel', function() {
			if (jqxhr) {
				jqxhr.abort();
			}
		...
		*	Cancel all uploads
		*/
		$(pxButton).on('click','#px-cancel', function(){
			stopUpload = true;
			if (jqxhr) {
				jqxhr.abort();
			}
